...
public class TileEntityHMS extends TileEntity implements ISidedInventory,
	...
	public void writeToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		par1NBTTagCompound.setCompoundTag("redstoneTank", rT);

		NBTTagCompound gT = new NBTTagCompound();
		glowstoneTank.writeToNBT(gT);
		par1NBTTagCompound.setCompoundTag("glowstoneTank", gT);
	...
	public void updateEntity() {
		...
		if (redstoneTank.getFluidAmount() >= FLUID_COST
				...
				&& (inv[0] == null || (inv[0].getItemDamage() == 4
						&& inv[0].itemID == EACommonProxy.itemMaterial.itemID && inv[0].stackSize < 64))) {
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
		if (resource.getFluid().getID() == TEFluids.fluidRedstone.getID())
			return redstoneTank.fill(resource, doFill);
		if (resource.getFluid().getID() == TEFluids.fluidGlowstone.getID())
			return glowstoneTank.fill(resource, doFill);
		if (resource.getFluid().getID() == TEFluids.fluidEnder.getID())
	...
	public boolean canFill(ForgeDirection from, Fluid fluid) {
		return fluid.getID() == TEFluids.fluidRedstone.getID()
				|| fluid.getID() == TEFluids.fluidGlowstone.getID()
				|| fluid.getID() == TEFluids.fluidEnder.getID();
	}
